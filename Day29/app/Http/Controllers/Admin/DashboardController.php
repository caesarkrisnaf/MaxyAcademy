...
class DashboardController extends Controller
{
    ...
    {
        $sales = Sale::all();
        return view('admin.reporting.reporting',["sales" => $sales]);
    }

    public function getAllDataProduct(){
        $sales = Sale::all();
        for ($i= 0; $i<count($sales); $i++){
            if ($sales[$i]['amount'] < 50000){
                $sales[$i]['price_range'] = 'less_50000';
            } else if ($sales[$i]['amount'] >= 50000 && $sales[$i]['amount'] < 99999 ){
                $sales[$i]['price_range'] = '_50000_99999';
            } else if ($sales[$i]['amount'] >= 100000 && $sales[$i]['amount'] < 999999 ){
                $sales[$i]['price_range'] = '_100000_999999';

            }else {
                $sales[$i]['price_range'] = 'more_1000000';
            }
            $sales[$i]['created_range'] = substr($sales[$i]['created_at'], 0, 7);
        }
